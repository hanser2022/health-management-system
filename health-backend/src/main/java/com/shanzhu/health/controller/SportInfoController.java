...
public class SportInfoController {
    ...
    public Unification<Map<String, Object>> getAllSportInfo() {
    ...
    public Unification<List<Map<String, Object>>> recommendSports(@RequestHeader("X-Token") String token) {
        try {
            User loginUser = jwtConfig.parseToken(token, User.class);
            if (loginUser == null || loginUser.getId() == null) {
                return Unification.fail(20003, "请先登录");
            }
            List<Map<String, Object>> recommendations = sportInfoService.recommendSports(loginUser.getId());
            if (recommendations.isEmpty()) {
                return Unification.success(recommendations, "请先录入身体信息以获取个性化推荐");
            }
            return Unification.success(recommendations);
        } catch (Exception e) {
            return Unification.fail(20003, "登录已过期，请重新登录");
        }
    }
